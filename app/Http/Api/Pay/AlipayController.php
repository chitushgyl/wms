...
//            $order_update['order_status'] = 6;
    ...
    public function addAlipay_notity(Request $request){
        ...
        if ($_POST['trade_status'] == 'TRADE_SUCCESS') {
            ...
            file_put_contents(base_path('/vendor/alipay2.txt'),$_POST);
...
//            $order_update['order_status'] = 6;
            ...
            $money['delete_flag']                = 'Y';
            $money['settle_flag']                = 'W';
            $tmsOrderCost = TmsOrderCost::where('order_id',$info>order_id)->select('self_id')->get();
            if ($tmsOrderCost){
...
//            $order_update['order_status'] = 6;
     ...
     * 加价微信支付回调
     * */
    public function addPricenotify(){
        ini_set('date.timezone','Asia/Shanghai');
        error_reporting(E_ERROR);
        $result = file_get_contents('php://input', 'r');
        $array_data = json_decode(json_encode(simplexml_load_string($result, 'SimpleXMLElement', LIBXML_NOCDATA)), true);
        if ($array_data['return_code'] == 'SUCCESS') {
            ...
            $pay['state'] = 'in';//支付状态
            $pay['self_id'] = generate_id('pay_');//微信账号
            $info = TmsSubOrder::where('self_id',$array_data['out_trade_no'])->select(['order_id','price','group_code','total_user_id','pay_state'])->first();
            $order = TmsOrder::where('self_id',$info->order_id)->select(['total_user_id','group_code','order_status','group_name','order_type','pay_state','add_price','price','total_money'])->first();
            if ($info->pay_state == 'Y'){
                echo 'success';
                return false;
            }
...
//        }
        ...
        $update['total_money'] = $order->total_money + $price*100;
