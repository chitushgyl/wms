...
class WmsRailwayController extends CommonController{
    /***    业务公司列表      /more/wmsRailway/wmsRailwayList
     */
    public function  wmsRailwayList(Request $request){
    ...
    //业务公司列表分页加载数据
    /***    业务公司分页      /more/wmsRailway/wmsRailwayPage
     */
    public function wmsRailwayPage(Request $request){
        /** 接收中间件参数**/
        ...
        $search=[
            ...
            ['type'=>'like','name'=>'railway','value'=>$railway],
        ];

        $where=get_list_where($search);

        $select=['self_id','railway','use_flag','group_name','group_code','create_time','update_time',
            'create_user_id','create_user_name','idle_flag'];
    ...
    public function createWmsRailway(Request $request){
        ...
        $data['info']=WmsRailway::where($where)->first();
        if($data['info']){
    ...
    /***    业务公司添加进入数据库      /more/wmsRailway/addWmsRailway
     */
    public function addWmsRailway(Request $request){
        ...
        if($validator->passes()){
            ...
            if($old_info){
                ...
                $id=WmsRailway::where($wheres)->update($data);
    ...
    /***    业务公司删除      /more/wmsRailway/delWmsRailway
     */
    public function delWmsRailway(Request $request,Status $status){
        $now_time=date('Y-m-d H:i:s',time());
        $operationing = $request->get('operationing');//接收中间件产生的参数
        $table_name='wms_railway';
        $medol_name='WmsRailway';
        $self_id=$request->input('self_id');
        $flag='delFlag';
        //$self_id='group_202007311841426065800243';
