...
use App\Models\Wms\WmsBillType;
...
class BillTypeController extends CommonController{
    /***    业务公司列表      /more/billType/billTypeList
     */
    ...
    //业务公司列表分页加载数据
    /***    业务公司分页      /more/billType/billTypePage
     */
    public function billTypePage(Request $request){
        /** 接收中间件参数**/
    ...
    public function createBillType(Request $request){
        ...
        $data['info']=WmsBillType::where($where)->first();
        if($data['info']){
    ...
    /***    业务公司添加进入数据库      /more/billType/addBillType
     */
...
//        dd($input);
        ...
        if($validator->passes()){
            ...
            $old_info=WmsBillType::where($wheres)->first();

            if($old_info){
                //dd(1111);
                $data['update_time']=$now_time;
                $id=WmsBillType::where($wheres)->update($data);
