...
use App\Models\Wms\WmsLogistic;
...
class LogisticController extends CommonController{
    /***    商品列表头部      /wms/logistic/logisticList
     */
    public function  logisticList(Request $request){
    ...
    /***    商品分页     /wms/logistic/logisticPage
     */
    public function logisticPage(Request $request){
...
//dd($button_info);
        ...
        switch ($group_info['group_id']){
            case 'all':
                $data['total']=WmsLogistic::where($where)->count(); //总的数据量
                $data['items']=WmsLogistic::where($where)
...
//                    ->select($select)
            ...
            case 'more':
                $data['total']=WmsLogistic::where($where)->whereIn('group_code',$group_info['group_code'])->count(); //总的数据量
                $data['items']=WmsLogistic::where($where)->whereIn('group_code',$group_info['group_code'])
                    ->offset($firstrow)->limit($listrows)->orderBy('self_id','desc')->orderBy('update_time', 'desc')
//                    ->select($select)
                    ->get();
                $data['group_show']='Y';
    ...
    /***    新建商品      /wms/logistic/createLogistic
     */
    public function createLogistic(Request $request){
        ...
        $data['info']=WmsLogistic::where($where)->first();
    ...
    /***    新建商品入库      /wms/logistic/addLogistic
     */
    public function addLogistic(Request $request){
        ...
        if($validator->passes()){
            ...
            $old_info=WmsLogistic::where($wheres)->first();

            if($old_info){
                //dd(1111);
                $data['update_time']=$now_time;
                $id=WmsLogistic::where($wheres)->update($data);
                ...
                $id=WmsLogistic::insert($data);
                $operationing->access_cause='新建物流公司';
                $operationing->operation_type='create';
    ...
    public function logisticUseFlag(Request $request,Status $status){
        ...
        $now_time=date('Y-m-d H:i:s',time());
        $operationing = $request->get('operationing');//接收中间件产生的参数
        $table_name='wms_logistic';
        $medol_name='WmsLogistic';
        $self_id=$request->input('self_id');
        $flag='useFlag';
    ...
    /***    商品删除      /wms/logistic/logisticDelFlag
     */
    public function logisticDelFlag(Request $request,Status $status){
        $now_time=date('Y-m-d H:i:s',time());
        $operationing = $request->get('operationing');//接收中间件产生的参数
        $table_name='wms_logistic';
        $medol_name='WmsLogistic';
