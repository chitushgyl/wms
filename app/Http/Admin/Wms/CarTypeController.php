...
class CarTypeController extends CommonController{
    /***    商品列表头部      /wms/carType/carTypeList
     */
    ...
    /***    商品分页     /wms/carType/carTypePage
     */
    public function carTypePage(Request $request){
...
//dd($button_info);
        ...
        switch ($group_info['group_id']){
            case 'all':
                $data['total']=WmsCarType::where($where)->count(); //总的数据量
                $data['items']=WmsCarType::where($where)
...
//                    ->select($select)
            ...
            case 'more':
                $data['total']=WmsCarType::where($where)->whereIn('group_code',$group_info['group_code'])->count(); //总的数据量
                $data['items']=WmsCarType::where($where)->whereIn('group_code',$group_info['group_code'])
                    ->offset($firstrow)->limit($listrows)->orderBy('self_id','desc')->orderBy('update_time', 'desc')
//                    ->select($select)
                    ->get();
                $data['group_show']='Y';
    ...
    public function createGood(Request $request){
        ...
        $data['info']=WmsCarType::where($where)->first();
    ...
    public function addGood(Request $request){
        ...
        if($validator->passes()){
            ...
            $old_info=WmsCarType::where($wheres)->first();

            if($old_info){
                //dd(1111);
                $data['update_time']=$now_time;
                $id=WmsCarType::where($wheres)->update($data);

                $operationing->access_cause='修改车型';
                $operationing->operation_type='update';


            }else{

                $data['self_id']=generate_id('CX');		//优惠券表ID
                ...
                $id=WmsCarType::insert($data);
                $operationing->access_cause='新建车型';
                $operationing->operation_type='create';
    ...
    public function carTypeUseFlag(Request $request,Status $status){
        ...
        $now_time=date('Y-m-d H:i:s',time());
        $operationing = $request->get('operationing');//接收中间件产生的参数
        $table_name='wms_car_type';
        $medol_name='WmsCarType';
        $self_id=$request->input('self_id');
        $flag='useFlag';
    ...
    /***    商品删除      /wms/carType/carTypeDelFlag
     */
    public function carTypeDelFlag(Request $request,Status $status){
        $now_time=date('Y-m-d H:i:s',time());
        $operationing = $request->get('operationing');//接收中间件产生的参数
        $table_name='wms_car_type';
        $medol_name='WmsCarType';
