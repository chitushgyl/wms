...
class LabourController extends CommonController{
    /***    业务公司列表      /wms/labour/labourList
     */
    ...
    //业务公司列表分页加载数据
    /***    业务公司分页      /wms/labour/labourPage
     */
    public function labourPage(Request $request){
        /** 接收中间件参数**/
...
//dump($wms_cost_type_show);
    ...
    /***    业务公司创建      /wms/labour/createLabour
     */
    public function createLabour(Request $request){
        ...
        $data['info']=WmsLabour::with(['InoutOtherMoney' => function($query)use($where1){
            $query->where($where1);
        }])->where($where)->first();
        if($data['info']){
    ...
    /***    业务公司添加进入数据库      /wms/labour/addLabour
     */
    public function addLabour(Request $request){
        ...
        $add_time           =$request->input('add_time');//寄存时间
        $state              =$request->input('state');//复核状态
        ...
        if($validator->passes()){
            ...
            $old_info=WmsLabour::where($wheres)->first();

            if($old_info){

                $operationing->access_cause='修改工人劳力费';
    ...
    /***    业务公司启用禁用      /wms/labour/labourUseFlag
     */
    public function depositUseFlag(Request $request,Status $status){
        $now_time=date('Y-m-d H:i:s',time());
        $operationing = $request->get('operationing');//接收中间件产生的参数
        $table_name='wms_labour';
        $medol_name='WmsLabour';
        $self_id=$request->input('self_id');
        $flag='useFlag';
    ...
    /***    业务公司删除      /wms/labour/labourDelFlag
     */
