...
class LibraryController extends CommonController{
    ...
    public function libraryPage(Request $request){
        ...
        switch ($group_info['group_id']){
            case 'all':
                $data['total']=WmsLibraryOrder::where($where)->count(); //总的数据量
                $data['items']=WmsLibraryOrder::with(['wmsLibrarySige' => function($query)use($where1){
                    $query->where($where1);
                }])->where($where)
...
//                $money->moneyCompute($data,$datalist,$now_time,$company_info,$user_info,'in');
    ...
    public function createLibrary(Request $request){
        ...
        $data['info']=WmsLibraryOrder::with(['wmsLibrarySige' => function($query) use($where1){
            ...
            ->with(['wmsGroup' => function($query) use($where1,$group_select){
                $query->where($where1);
                $query->select($group_select);
            }])
            ->with(['wmsContract' => function($query) use($where1,$contract_select){
                $query->where($where1);
                $query->select($contract_select);
            }])
            ->where($where)->first();
        if($data['info']){
