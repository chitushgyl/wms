...
//        dump($group_info['group_code']);
        ...
        if($user_info->menu_id== 'all'){
            $menu_info=SystemMenuNew::with(['allChildren' => function($query)use($where_children,$select) {
            ...
            $menu_info=SystemMenuNew::with(['allChildren' => function($query)use($where_children,$select,$menu_id) {
                $query->where($where_children);
                $query->select($select);
                $query->orderBy('sort','asc');
